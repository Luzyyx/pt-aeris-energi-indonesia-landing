...
export default function ImageFanCards({ cards, className = '' }) {
  ...
  return (
    <div className={`fan-showcase ${className}`}>
      <div ref={containerRef} className="fan-layout">
        {cards.map((card, index) => {
          const content = (
            <>
              <img src={card.imgUrl} loading="lazy" alt={card.alt || card.title || `Kartu ${index + 1}`} />
          ...
          return card.linkUrl ? (
            <a key={card.title || index} href={card.linkUrl} className="fan-card">
              {content}
            </a>
          ) : (
            <div key={card.title || index} className="fan-card">
              {content}
            </div>
          );
        })}
      ...
      {needsPagination ? (
        <div className="fan-controls">
          ...
          <div className="fan-dots">
            {cards.map((_, index) => (
              <span key={index} className={index === centerIndex ? 'active' : ''} />
            ))}
          </div>
